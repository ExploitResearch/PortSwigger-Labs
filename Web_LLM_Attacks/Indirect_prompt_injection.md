...
This lab demonstrates a vulnerability in an application that integrates a Large Language Model (LLM). The LLM has access to APIs or functionality that can be exploited through carefully crafted prompts or indirect injection.
...
3. Test if the LLM processes untrusted input from data sources (indirect prompt injection)
...
- Indirect prompt injection via data sources is a real attack vector
- LLM output must be sanitized before rendering
- AI agents need human-in-the-loop for destructive actions
